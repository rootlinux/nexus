...
from app.models.waitlist_application import WaitlistApplication, WaitlistApplicationStatus
...
from app.services.notifications import (
    create_account_banned_notification,
    create_account_frozen_notification,
    create_account_suspended_notification,
    create_admin_post_removed_notification,
)
...
from app.schemas.waitlist import WaitlistApplicationUpdate, WaitlistApplicationResponse, WaitlistApplicationListResponse, WaitlistInviteResponse
...
async def get_admin_session(
    ...
    return AdminSessionRead(
        ...
        capabilities=StaffCapabilityRead(
            ...
            can_read_waitlist=staff_has_capability(current_admin, Capability.WAITLIST_READ),
            can_manage_waitlist=staff_has_capability(current_admin, Capability.WAITLIST_MANAGE),
            can_read_audit_log=staff_has_capability(current_admin, Capability.AUDIT_READ),
        ),
    )
...
async def ban_user(
    ...
    await create_account_banned_notification(
        db,
...
async def suspend_user(
    ...
    await create_account_suspended_notification(
        db,
        recipient_user_id=user.id,
        actor_user_id=current_admin.id,
    )
...
async def freeze_user(
    ...
    await create_account_frozen_notification(
        db,
        recipient_user_id=user.id,
        actor_user_id=current_admin.id,
    )
...
async def delete_post_admin(
    ...
    await create_admin_post_removed_notification(
        db,
...
async def list_waitlist_applications(
    ...
    limit: int = Query(50, ge=1, le=100),
    current_admin: User = Depends(require_capability(Capability.WAITLIST_READ)),
    db: AsyncSession = Depends(get_db),
):
    await enforce_rate_limits(request, _admin_read_policies(current_admin.id, "waitlist"))
    ...
    if status_filter:
        try:
            status_enum = WaitlistApplicationStatus(status_filter)
            query = query.where(WaitlistApplication.status == status_enum)
            count_query = count_query.where(WaitlistApplication.status == status_enum)
        except ValueError:
            pass
    ...
    if search:
        ...
        search_filter = (
            or_(
                WaitlistApplication.full_name.ilike(search_pattern),
                WaitlistApplication.contact.ilike(search_pattern),
                WaitlistApplication.preferred_username.ilike(search_pattern),
            )
        )
    ...
    query = query.order_by(WaitlistApplication.created_at.desc())
    query = query.limit(limit).offset((page - 1) * limit)

    result = await db.execute(query)
    applications = result.scalars().all()

    return WaitlistApplicationListResponse(
        applications=[
            WaitlistApplicationResponse.model_validate(app) for app in applications
        ],
        total=total,
        limit=limit,
        offset=(page - 1) * limit,
...
async def update_waitlist_application(
    request: Request,
    application_id: int,
    update_data: WaitlistApplicationUpdate,
    current_admin: User = Depends(require_capability(Capability.WAITLIST_MANAGE)),
    db: AsyncSession = Depends(get_db),
):
    await enforce_rate_limits(request, _admin_read_policies(current_admin.id, "waitlist"))

    result = await db.execute(
        select(WaitlistApplication).where(WaitlistApplication.id == application_id)
    )
    ...
    if update_data.status is not None:
        try:
            application.status = WaitlistApplicationStatus(update_data.status)
        except ValueError:
            raise HTTPException(
                status_code=status.HTTP_400_BAD_REQUEST,
...
@router.post("/waitlist/{application_id}/invite", response_model=WaitlistInviteResponse)
async def create_waitlist_invite(
    request: Request,
    application_id: int,
    current_admin: User = Depends(require_capability(Capability.WAITLIST_MANAGE)),
    db: AsyncSession = Depends(get_db),
    ...
    if application.invite_id is not None:
        existing_invite_result = await db.execute(
            select(InviteCode).where(InviteCode.id == application.invite_id)
        )
        existing_invite = existing_invite_result.scalar_one_or_none()
        if existing_invite:
            raise HTTPException(
                status_code=status.HTTP_409_CONFLICT,
                detail="An invite already exists for this application",
            )

    code = _generate_invite_code(32)
    while True:
    ...
    internal_note = f"Waitlist application #{application_id} - approved by admin"
    ...
    application.invite_id = new_invite.id
    application.status = WaitlistApplicationStatus.APPROVED

    await write_audit_log(
        db,
        action="waitlist_invite_created",
        actor_user=current_admin,
        target_type="invite",
        target_id=new_invite.id,
        after={
            "waitlist_application_id": application_id,
...
@router.get("/waitlist/{application_id}/invite", response_model=WaitlistInviteResponse)
async def get_waitlist_invite(
    request: Request,
    application_id: int,
    current_admin: User = Depends(require_capability(Capability.WAITLIST_READ)),
    db: AsyncSession = Depends(get_db),
    ...
    if application.invite_id is None:
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
            detail="No invite exists for this application",
        )
