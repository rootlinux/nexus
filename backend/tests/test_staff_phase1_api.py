...
class FakePhase1DB:
    ...
    def __init__(self):
        ...
        self._session = RefreshToken(id=self.SESSION_ID, user_id=0, token_hash="x", expires_at=_expires, revoked=False, mfa_satisfied=False)
    ...
    async def execute(self, statement):
        ...
        if entity is User:
            if self.target_user is not None:
                self._session.user_id = self.target_user.id
        ...
        if entity is RefreshToken:
            # Session lookup by id → scalar; user token list → list
            if "refresh_tokens.id =" in statement_text:
