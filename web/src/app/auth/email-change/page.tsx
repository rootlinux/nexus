...
import { completeEmailChange } from '../../../lib/api'
...
function EmailChangePageContent() {
  ...
  const [message, setMessage] = useState('Checking your email change...')

  useEffect(() => {
    const run = async () => {
      if (!token) {
        setStatus('error')
        setMessage('This email-change link is incomplete.')
        return
      }

      try {
        const response = await completeEmailChange(token)
        setStatus(response.status === 'verified' ? 'verified' : 'error')
    ...
    <main style={{ minHeight: '100vh', display: 'grid', placeItems: 'center', backgroundColor: tokens.colors.bg, padding: '24px' }}>
      <section style={{ width: '100%', maxWidth: '480px', borderRadius: '14px', border: `1px solid ${tokens.colors.border}`, backgroundColor: tokens.colors.surface, padding: '28px', display: 'grid', gap: '16px' }}>
        ...
        <h1 style={{ margin: 0, color: tokens.colors.textPrimary, fontSize: '28px', lineHeight: 1.15 }}>
          {status === 'loading' ? 'Checking your link' : status === 'verified' ? 'Email updated' : 'Email change could not be completed'}
        </h1>
