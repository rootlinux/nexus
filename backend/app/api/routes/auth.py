...
async def login(
    ...
    if not has_webauthn and staff_session_requires_security_key(user):
        raise HTTPException(
            status_code=status.HTTP_403_FORBIDDEN,
            detail="Admin accounts require a security key. Register one first.",
        )
...
async def issue_admin_webauthn_recovery_token_route(
    ...
    if not settings.ENABLE_ADMIN_WEBAUTHN_RECOVERY:
    ...
    if configured_identifier is None:
