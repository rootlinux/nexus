...
Add or update the mail HTML assertions so they verify the email logo source is derived from the app’s production base URL and no `example.com` asset is present.
...
- legacy top-level ExampleApp icon paths are not used for install metadata
...
Run: `rg -n "ExampleApp|example.com|/icon-192.png|/icon-512.png|/apple-touch-icon.png" backend web`
